...
- **Output:** `../out/waldrive-demo.mp4` — 1920×1080 native (authored and rendered 1:1, no upscale), 30fps, H.264 + AAC, ~3:42.
...
| 0:43–0:49     | 02 Grab gas         | `assets/clips/02-faucet.mp4` (in-app faucet, devnet) |
| 0:49–1:21     | 03 Multi-upload     | `assets/clips/03-multi-upload.mp4`                |
| 1:21–1:34     | 04 **Verify**       | `assets/clips/04-verify.mp4` (slow zoom on proof) |
| 1:34–1:59     | 05 Organize         | `assets/clips/05-organize.mp4`                    |
| 1:59–2:41     | 06 Versions         | `assets/clips/06-versions.mp4`                    |
| 2:41–2:58     | 07 Storage          | `assets/clips/07-storage.mp4`                     |
| 2:58–3:13     | 08 Accounts         | `assets/clips/08-accounts.mp4`                    |
| 3:13–3:24     | MCP                 | text scene (the agent write-path)                 |
| 3:24–3:42     | Conclusion          | text scene + vision                               |
...
```bash
cd video/hf
npx hyperframes render . --quality high --fps 30 --resolution landscape --crf 17 --output ../out/waldrive-demo.mp4
```

> `--resolution landscape` is 1920×1080 — native, no upscale. The composition is
> authored at 1080p; rendering at `landscape-4k` (the old setting) made Chrome
> capture at 2× DPR and upscaled 1080p footage to 4K, which softened every UI
> clip. 1:1 keeps the screen recordings sharp.
...
brand-name mishears Whisper makes (WalDrive, Sui, backend, re-fetches, devnet,
testnet, captcha-free). Add to `PHRASE_FIX` for single-token fixes; for a brand
name Whisper splits across two tokens (e.g. "WalDrive's" → "while Drive's", which
falls across a phrase boundary that `PHRASE_FIX` can't reach), add the bigram to
`WORD_FIX` — it merges the tokens before grouping.
